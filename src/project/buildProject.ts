...
import { pickCameraPresetForModelCount } from './presets';
import type {
  AnimaStageProject,
  AnimaStageCameraBlock,
  CameraPresetId,
} from './types';

function cameraPresetFromState(
  appState: AppState,
  modelCount: number
): CameraPresetId {
...
export function buildAnimaStageProject(
  appState: AppState,
...
/** Upgrade legacy v1 local storage project → v2. */
export function migrateV1ToV2(v1: SavedProjectV1): AnimaStageProject {
  return {
    format: 'animastage',
    version: 2,
    name: v1.name,
    ...
    motions: v1.models.map((m, modelIndex) => ({
    ...
    camera: {
      preset: pickCameraPresetForModelCount(v1.models.length),
      mode: v1.cameraMode,

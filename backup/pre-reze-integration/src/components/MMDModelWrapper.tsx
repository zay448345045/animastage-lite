...
export default function MMDModelWrapper({
  ...
  useEffect(() => {
    ...
    const evaluated = evaluateTimelineAtFrame(
      timelineKeyframes,
      frame,
      timelineLiveRef.current
    );
    applyTimelineToSkinnedMesh(mesh, evaluated);
    syncSkeletonBeforePhysics(mesh);
  }, [timelineKeyframes, activeTemplateId, animationReady, currentFrame, isPlaying, vmdPlaybackEnabled]);
  ...
  useFrame((_, delta) => {
    ...
    const helper = helperRef.current;
    const meshState = getHelperMeshState(helper, currentMesh);
    const hasManualTimeline = timelineKeyframesRef.current.length > 0;
    const hasVmd =
    ...
    // Cloth / hair / skirt — Bullet rigid bodies (юбка, волосы, аксессуары).
